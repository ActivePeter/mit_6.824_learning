...
func TestInitialElection2A(t *testing.T) {
	...
	log.Println("check leader constant")
...
func TestReElection2A(t *testing.T) {
	...
	//fmt.Printf("%v leader is\n", leader1)
	// if the leader disconnects, a new one should be elected.
	println("  test disco", leader1)
	cfg.disconnect(leader1)
	cfg.checkOneLeader()
	//fmt.Printf("%v leader is\n", l2)
	// if the old leader rejoins, that shouldn't
	// disturb the new leader.
	println("  test co", leader1)
	cfg.connect(leader1)
	leader2 := cfg.checkOneLeader()

	// if there's no quorum, no leader should
	// be elected.
	println("  test disco", leader2)
	println("  test disco", (leader2+1)%servers)
	...
	println("  test co", (leader2+1)%servers)
	cfg.connect((leader2 + 1) % servers)
	cfg.checkOneLeader()

	// re-join of last node shouldn't prevent leader from existing.

	println("  test co", leader2)
	cfg.connect(leader2)
	cfg.checkOneLeader()
